...
class Vacuum extends Base {
  ...
  setVacuum(config, _device) {
    const sid = config.sid
    const uuid = UUIDGen.generate(`Mijia-Vacuum@${sid}`)
    ...
    if (!accessory) {
      ...
      accessory.on('identify', function(_paired, callback) {
        callback()
        const d = this.devices[sid]
        if (d != undefined) {
          d.find() // find me,will make the device give off a sound.
        }
      })
    ...
    // bind
    const setter = service_fan.getCharacteristic(Characteristic.On).listeners('set')
    if (!setter || setter.length == 0) {
      ...
      service_fan
        ...
        .on('set', (value, callback) => {
          const d = this.devices[sid]
          if (d != undefined && value) {
            d.start()
          } else {
            d.pause()
            d.charge() // go to home(app_stop && app_charge)
          }
          callback(null, value)
        })
      service_fan
        ...
        .on('set', (value, callback) => {
          ...
          if (device != undefined) {
            const speeds = [0, 38, 60, 77, 90]
            for (const item in speeds) {
              if (value <= item) {
                value = item
                break
              }
